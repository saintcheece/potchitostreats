<?php
    ...
    $stmt = $conn->prepare("SELECT o.oID, p.pID, p.pType, p.pName, p.pPrice,  o.oQty, ((p.pPrice + COALESCE(cf.cfPrice, 0) + COALESCE(cs.csPrice, 0))) AS total, p.pPrepTime
                                    FROM orders o
    ...
    'body' => '{
        "data": {
            "attributes": {
                ...
                "cancel_url": "http://localhost/potchitos/public/pages/order.php",
                "line_items": ' . json_encode($line_items) . ',
                "payment_method_types": [
                    "gcash"
                ],
                "success_url": "http://localhost/potchitos/controller/check_balance.php?transactionID=' . $_GET['transactionID'] . '",
